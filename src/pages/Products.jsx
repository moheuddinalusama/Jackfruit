...
import { Star, ShoppingBag, Search, SlidersHorizontal, ArrowRight } from "lucide-react";
...
export default function Products() {
  ...
  const categories = ["All", "Accessories", "Apparel", "Skincare", "Hydration", "Electronics"];
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 text-neutral">
            <div>
               ...
               <h1 className="text-5xl md:text-6xl font-display font-black uppercase italic leading-none">Summer Essentials</h1>
            </div>
            <div className="flex items-center gap-4 text-sm font-bold text-neutral/40 uppercase tracking-widest">
               <span className="text-neutral">{filteredProducts.length}</span> Products Found
            </div>
          </div>
        </div>
      </section>
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-8 py-3 rounded-2xl text-[10px] uppercase font-black tracking-widest transition-all min-w-max ${
                  filter === cat 
                  ? "bg-neutral text-white shadow-2xl shadow-neutral/30 scale-105" 
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 gap-y-24">
            {filteredProducts.map((product, idx) => (
              ...
              >
                <Link to={`/products/${product.id}`} className="relative aspect-[3/4] rounded-[3.5rem] overflow-hidden bg-[#F5F4F3] mb-10 border border-neutral/5 shadow-sm group-hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] transition-all duration-1000 ease-out block">
                  <img 
                    src={product.image} 
                  ...
                  <div className="absolute bottom-8 left-8 right-8 translate-y-[250%] group-hover:translate-y-0 transition-transform duration-700 ease-in-out">
                     <div className="w-full py-6 bg-white text-neutral rounded-[2.5rem] font-black text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 hover:bg-neutral hover:text-white transition-all shadow-2xl">
          ...
          {filteredProducts.length === 0 && (
             <div className="text-center py-20">
                ...
                <h2 className="text-2xl font-display font-bold text-neutral/80">No items found in this section</h2>
                <p className="text-neutral/40 mt-2">Try selecting a different category or exploring our new arrivals.</p>
                <button onClick={() => setFilter("All")} className="mt-8 px-8 py-4 bg-neutral text-white rounded-2xl font-bold text-sm">Clear Filter</button>
             </div>
          )}
        </div>
      </section>
